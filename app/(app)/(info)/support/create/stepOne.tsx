...
import {_SupportTicketType} from "@/lib/@type";
import formSupportStore from "@/lib/store/formSupportStore";
import {router} from "expo-router";

export default function StepOneScreen (){

    const {setStepOne} = formSupportStore()
           ...
           <YStack gap="$4" marginBottom="$4">
            ...
            <YStack gap="$4">
                <TouchableOpacity onPress={() => nextStep(_SupportTicketType.PAYMENT)}>
                    <Card paddingHorizontal={32} paddingVertical={24} gap={"$4"} bordered elevation={"$1"} backgroundColor={DefaultColor.white}>
                        <MaterialCommunityIcons name="hand-coin-outline" size={40} color="black" />
                ...
                <TouchableOpacity onPress={() => nextStep(_SupportTicketType.TRANSACTION)}>
                    <Card paddingHorizontal={32} paddingVertical={24} gap={"$4"} bordered elevation={"$1"} backgroundColor={DefaultColor.white}>
                ...
                <TouchableOpacity onPress={() => nextStep(_SupportTicketType.TECHNICAL)}>
                    <Card paddingHorizontal={32} paddingVertical={24} gap={"$4"} bordered elevation={"$1"} backgroundColor={DefaultColor.white}>
