...
import {Button, Paragraph, XStack, YStack, Separator} from "tamagui";
...
export default function InfoScreen() {
    ...
    const authData = useAuthStore((state) => state.auth_data);
                        ...
                        <XStack gap="$2" alignItems="center">
                            <View
                                style={{
                                    ...
                                    borderRadius: 32,
                                    backgroundColor:
                                        authData?.user.status === _VerifyUserStatus.ACTIVE
                                            ? "#EEFBF3"
                                            : (authData?.user.status === _VerifyUserStatus.INACTVE ? "#FDF1EC" :
                                                    (authData?.user.status === _VerifyUserStatus.WAITING ? "#FFFBED" : "#ccc")
                                            ),
                                }}>
                                ...
                                {authData?.user.status === _VerifyUserStatus.INACTVE &&
                                    <Paragraph fontWeight={500} fontSize={12} color="#814441">Chưa xác minh</Paragraph>
                                }
                                {authData?.user.status === _VerifyUserStatus.WAITING &&
